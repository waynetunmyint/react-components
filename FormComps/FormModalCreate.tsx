...
export default function FormModalCreate({ dataSource, fields, imageSize, onSuccess, hideBackButton = false }: Props) {
  ...
  useEffect(() => {
    fields.forEach(async (field) => {
      ...
      if (field.defaultValue) {
        setFormData((prev: any) => ({ ...prev, [toCamelCase(field.fieldName)]: field.defaultValue }));
      }

      // Load last selection for dropdowns from localStorage if no defaultValue
      if ((field.type === "dropdown" || field.type === "hundredDropdown" || field.type === "iconDropdown") && !field.defaultValue) {
        const storedValue = localStorage.getItem(`LastSelect_${dataSource}_${field.fieldName}`);
        if (storedValue) {
          setFormData((prev: any) => ({ ...prev, [toCamelCase(field.fieldName)]: storedValue }));
        }
      }

      // Default Radio/RadioPayment to 1 (Active) if not set
      if ((field.type === 'radio' || field.type === 'radioPayment') && field.defaultValue === undefined) {
        setFormData((prev: any) => ({ ...prev, [toCamelCase(field.fieldName)]: "1" }));
      }
    });
  ...
  const handleChange = (fieldName: string, value: any) => {
    ...
    setFormData((p: any) => ({ ...p, [camel]: value }));

    // Save last selection for dropdowns to localStorage
    const field = fields.find(f => f.fieldName === fieldName);
    if (field?.type === "dropdown" || field?.type === "hundredDropdown" || field?.type === "iconDropdown") {
      localStorage.setItem(`LastSelect_${dataSource}_${fieldName}`, String(value));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fields.forEach((field) => {
        ...
        switch (field.type) {
          case "dropdown":
            // Normalize: treat '0' or 0 as empty string so DB helper converts to NULL
            const ddlVal = value === '0' || value === 0 ? '' : (value || '');
            console.log(`[FormModalCreate] dropdown ${field.fieldName}: camelKey=${camel}, value=${value}, normalized=${ddlVal}, appendKey=${camel}Id`);
            submitData.append(camel + "Id", ddlVal);
            break;
          case "multiple":
