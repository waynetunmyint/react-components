...
export function DatabaseCrudComp(config: CrudConfig) {
  ...
  const GET = async () => {
    try {
      ...
      for (const f of oneToManyFields) {
        ...
        sql += `, ${relatedTable}.Title AS ${relatedTable}Title`;
        sql += `, ${relatedTable}.Thumbnail AS ${relatedTable}Thumbnail`;
      }
      ...
      if (config.manyToManyJoin) {
        ...
        sql += `,
          GROUP_CONCAT(${config.manyToManyJoin.joinTable}.${config.manyToManyJoin.relatedKey}) AS ${singular}Ids,
          GROUP_CONCAT(${config.manyToManyJoin.relatedTable}.${config.manyToManyJoin.relatedNameField}) AS ${config.manyToManyJoin.relatedTable}
        `;
      }
  ...
  const POST = async (req: NextRequest) => {
    ...
    try {
      ...
      const sql = `INSERT INTO ${config.table} (${columns.join(",")}) VALUES (${columns.map(() => "?").join(",")})`;
