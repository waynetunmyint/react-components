...
export default function DisplayBlockFour({ dataSource, headingTitle, subHeadingTitle }: Props) {
      ...
      <div className="max-w-7xl mx-auto">
        {headingTitle && (
          <div className="mb-6 text-center">
            <h3 className={`?? "text-gray-900"} text-2xl md:text-3xl font-bold`}>{headingTitle}</h3>
            {subHeadingTitle && <p className={`${"text-sm md:text-base text-white"} max-w-3xl mx-auto mt-3`}>{subHeadingTitle}</p>}
          </div>
        )}
        ...
        <div className="divide-y overflow-hidden">
          {loading ? (
            ...
            items.map((item, idx) => {
              ...
              return (
                <div key={item.Id || idx} className="p-4 flex items-start gap-4 mb-2 bg-white rounded-lg shadow-sm hover:shadow-md transition">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs text-gray-500 flex items-center gap-2"><Calendar size={14} /><span>{formatDate(created)}</span></div>
                    </div>
