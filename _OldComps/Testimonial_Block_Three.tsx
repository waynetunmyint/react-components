...
export default function TestimonialBlockThree({ dataSource, headingTitle, subHeadingTitle }: Props) {
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <Star key={i} size={14} className={`${i <= valid ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"} transition-colors`} />
      );
    }
    return stars;
  };
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-100 flex-shrink-0">
                      {imageUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-gray-300"> <Quote size={24} /> </div>
                      )}
                    </div>
