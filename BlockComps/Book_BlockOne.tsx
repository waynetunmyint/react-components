            ...
            items.map((item, idx) => {
              ...
              if (viewMode === "list") {
                return (
                  <div
                    ...
                    className="group flex gap-4 bg-white rounded-2xl p-4 shadow-sm hover:shadow-md border border-gray-100 transition-all cursor-pointer"
                  >
                    <div className="w-32 h-32 rounded-xl overflow-hidden flex-shrink-0">
                      <img src={imageUrl} alt={title} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 flex flex-col justify-center">
                      <h3 className="font-semibold text-gray-900 line-clamp-1 group-hover:text-[var(--theme-primary-bg)]">{title}</h3>
                      {bookAuthorTitle && <p className="text-gray-500 text-sm">By {bookAuthorTitle}</p>}
                    </div>
                  </div>
                );
              }
              ...
              return (
                <div
                  key={item.Id || idx}
                  onClick={() => handleNavigation(item.Id)}
                  className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all cursor-pointer group flex flex-col"
                >
                  <div className="aspect-square bg-gray-50 overflow-hidden">
                    <img src={imageUrl} alt={title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  </div>
                  <div className="p-3 text-center">
                    <h3 className="text-sm font-medium text-gray-900 line-clamp-1 group-hover:text-[var(--theme-primary-bg)]">{title}</h3>
                    {bookAuthorTitle && <p className="text-xs text-gray-500 line-clamp-1 mt-1">{bookAuthorTitle}</p>}
                    {item.Price && (
                      <p className="text-xs font-bold text-[var(--theme-primary-bg)] mt-2">
